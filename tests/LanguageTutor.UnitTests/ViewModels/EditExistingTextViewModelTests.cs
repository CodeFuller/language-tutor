...
namespace LanguageTutor.UnitTests.ViewModels
{
	...
	public class EditExistingTextViewModelTests
	{
		...
		public async Task Load_ForFirstCall_InitializesViewModelCorrectly()
		{
			...
			mocker.GetMock<IDictionaryService>()
		...
		public async Task SaveChanges_WhenCreatePronunciationRecordIsTrueAndPronunciationRecordWasChanged_UpdateLanguageTextWithNewPronunciationRecord()
		{
			...
			var dictionaryServiceMock = mocker.GetMock<IDictionaryService>();
			...
			dictionaryServiceMock.Setup(x => x.GetLanguageTexts(TestLanguage, It.IsAny<CancellationToken>())).ReturnsAsync(TestExistingLanguageTexts);
			dictionaryServiceMock.Setup(x => x.UpdateLanguageText(It.IsAny<LanguageText>(), It.IsAny<LanguageTextData>(), It.IsAny<CancellationToken>())).ReturnsAsync(updatedLanguageText);
			...
			dictionaryServiceMock.Verify(x => x.UpdateLanguageText(originalLanguageText, It.Is<LanguageTextData>(data => verifyLanguageTextData(data)), It.IsAny<CancellationToken>()), Times.Once);
		...
		public async Task SaveChanges_WhenNoteForTextWithoutDuplicatesIsCleared_SavesChangesCorrectly()
		{
			...
			var dictionaryServiceMock = mocker.GetMock<IDictionaryService>();
			dictionaryServiceMock.Setup(x => x.GetLanguageTexts(TestLanguage, It.IsAny<CancellationToken>())).ReturnsAsync(new[] { testEditedLanguageText });
			...
			dictionaryServiceMock.Verify(x => x.UpdateLanguageText(testEditedLanguageText, It.Is<LanguageTextData>(data => verifyLanguageTextData(data)), It.IsAny<CancellationToken>()), Times.Once);
		}
